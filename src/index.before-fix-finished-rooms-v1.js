...
io.on("connection", (socket) => {
  ...
  socket.on("room:addBot", ({ roomId }) => {
    ...
    // إصلاح ROOM_FULL:
    // إذا أنا داخل الغرفة والبوت موجود، لا ترسل خطأ. فقط رجع الغرفة الحالية.
    if (me && existingBot) {
      socket.join(room.id);
      socket.emit("room:joined", publicRoom(room, socket.user.id));
      emitRoom(room);
    ...
    // إذا الغرفة ممتلئة وأنا موجود فيها، لا تعتبره خطأ.
    if (room.players.length >= 2 && me) {
      socket.join(room.id);
      socket.emit("room:joined", publicRoom(room, socket.user.id));
      emitRoom(room);
      return;
    ...
    // إذا الغرفة ممتلئة وأنا لست من اللاعبين، هنا فقط ROOM_FULL صحيح.
